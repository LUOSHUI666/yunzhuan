...
- 项目目录结构：`index.html` / `assets/` / `.gitignore` / `README.md` / `CHANGELOG.md` / `CLAUDE.md`
- `index.html` 六模块注释骨架（仅注释，无实现代码）

**待实现（按顺序）：**
1. 主题样式体系（CSS 变量 + 背景星图 + 字体回退）
2. 八卦罗盘（SVG/CSS 纹样 + 中心隶书 MBTI 文字 + 左右青铜箭头）
3. 环形 16 型全览面板（点击罗盘弹出圆形菜单）
4. 六张符箓令牌（浮动动画 + 悬停光晕）
5. 启卦动效（粒子消散 + 飞升转场）
