...
融合道家符箓美学 × MBTI 性格类型的**单文件离线 Demo**。用户旋转罗盘选择本命类型，点击符箓令牌启卦，获得道家哲思解读与虚拟问道时辰。核心价值：以仪式化交互替代生硬测评，用庄子式空灵哲思给予内心指引。
...
- **Step 1 入境**：CSS 主题体系（12 色 + 2 字体）+ 3 层星图旋转 + 八卦罗盘 + 中心隶书文字 + 左右青铜箭头切换 16 型 + 键盘左右键
- **Step 2 问道（前半）**：`#stage` 纵向弹性布局 + `.compass-row` 包裹罗盘行 + 六张符箓令牌（80×180px 竖向朱砂隶书、独立浮动动画 ±6px 周期 3~4s、悬停朱砂光晕）
- `index.html` ~650 行，六模块架构完整运行

**待实现（按顺序）：**
1. ~~主题样式体系~~ ✅
2. ~~八卦罗盘~~ ✅
3. ~~六张符箓令牌~~ ✅
4. 环形 16 型全览面板（点击罗盘弹出圆形菜单，下一步优先）
5. 启卦动效（粒子消散 + 飞升转场）
6. 解读卡片（竹简纹理 + 打字机正文 + 时辰）
7. TOKEN_DATA 文案数据（最少 4 类型 × 6 令牌 × 2 变体 = 48 组）
8. 归真重置 + 全局打磨
...
|------|------|------|
| 背景 | `#0D0D14` | 深色星图基底 |
| 暗金 | `#8B7355` / `#B8860B` / `#DAA520` | 罗盘线条 |
| 朱砂 | `#C41E3A` / `#E23D3D` | 令牌文字、悬停发光 |
| 青铜 | `#6B3A2A` / `#8B5A3C` | 箭头按钮、回纹装饰 |
| 铜绿 | `#4A5A3E` / `#2E3828` | 令牌青铜锈底 |
...
1. **入境**：页面加载，星图旋转，罗盘自转显示默认类型 INFP，左右箭头可切换 16 型
2. **问道**：六张令牌独立浮动（±6px，周期 3~4s），悬停时朱砂字发光
3. **启卦**：点击令牌 → 20 个金色粒子 0.8s 消散 → 其他令牌淡出 → 选中令牌飞升中央 → 罗盘加速
4. **观象**：令牌展开为竹简卡片 → 标签"INFP · 逍遥散人" → 时辰打字机 80ms/字 → 正文打字机 40~60ms/字 → 底部淡出"道法自然，不必执着"
5. **归真**：点击卡片外空白 → 卡片淡出 → 令牌归位 → 罗盘恢复 → 可重新启卦
